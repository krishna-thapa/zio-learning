...
import zio.streams.ZioStreamIntro.Environment
...
object ZioStreamError extends ZIOAppDefault {
  ...
  val sink: ZSink[Any, Nothing, String, Nothing, String] = ZSink.collectAll[String].map(chunk => chunk.mkString("-"))

  // 99 is higher than the length of our data, so we won't fail
  val nonFailingStream: ZStream[Any, IOException, String] =
    ZStream
      .fromInputStream(new FakeInputStream(12, 99, new IOException("Something went bad")), chunkSize = 1)
  ...
  val failingStream: ZStream[Any, IOException, String] =
    ZStream
      .fromInputStream(new FakeInputStream(10, 5, new IOException("Something went bad")), chunkSize = 1)
      .map(byte => new String(Array(byte)))

  // will fail, but the error does not match the ZStream error channel
  val defectStream: ZStream[Any, IOException, String] =
    ZStream
      .fromInputStream(new FakeInputStream(10, 5, new IndexOutOfBoundsException("")), chunkSize = 1)
  ...
  val recoveredEffect: ZStream[Any, Throwable, String] = failingStream.orElse(recoveryStream)

  // all elements from the original stream = Left, all elements from the fallback = Right
  val recoveredEffectWithEither: ZStream[Any, Throwable, Either[String, String]] =
    failingStream.orElseEither(recoveryStream)
  val debugRecoveredEffectWithEither = recoveredEffectWithEither.run(ZSink.foreach(ZIO.succeed(_).debug("Either")))
  ...
  val errorContainedDebug = errorContained.run(ZSink.collectAll[Either[IOException, String]]).debug
